...
def load_model_tokenizer(path, dev_map='cuda:0'):
    device_map = split_model(path) if device_map is None else dev_map
...
def load_prompt(yaml_path, prompt_type):
    prompt_path = os.path.join(yaml_path, prompt_type)
...
if __name__ == "__main__":
    ...
    # forward pass
    acts = accessor.compute_mlp_activate(activations, num_layers=48)
    #TODO
    sample_idx = 'get_importance_method'
    pruned_model = accessor.apply_mlp_prune()
